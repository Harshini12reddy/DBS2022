...
public class App {
	...
	public static void main(String[] args) {
		...
		if(app.validateCustomer(email,customers)) {
			System.out.println("please choose to purchase");
			...
			while(true) {
				...
				for(Product product:products) {
					if(product.getProdid()==pid) {
						LineItem item= new LineItem(id++,invoice,product,qty);
						items.add(item);
						break;
					}
				}
				...
				int total=0;
				System.out.println("-----------------------------------------------------------------");
				System.out.println("Invoice No "+invoice.getInvo());
				...
				for(LineItem item : invoice.getLineItems()) {
					...
					System.out.format("%5s%10s%10s%10s%10s\n",
					  item.getId(),item.getProduct().getName(),item.getProduct().getPrice(),item.getQuantity(),sum);
					System.out.println("-----------------------------------------------------------------");
					  total+=sum;
